...
if __name__ == "__main__":
    print("=" * 70)
    print("SIMPLE INFO EXTRACTOR - TEST RESULTS")
    print("=" * 70)
    ...
    country_tests = [
        ["France ,Italy ,Turkey"],
        ["PL, TH, CN"],
        ["3. Menşe Ülkesi", "Country of Origin", "TURKEY"],
        ["CHINA - TURKEY"],
    ]
    
    for test in country_tests:
        result = extract_info(test, information_type='country')
        print(f"\nInput:  {test}")
    ...
    weight_tests = [
        ["7. Miktar", "Quantity", "Quantité", "5.236,00 KG"],
        ["7. Miktar", "Quantity", "Quantité", "850,00 KG"],
        ["7. Ilość", "Quantity", "Quantité", "0,70 KG", "131,00 KG", "345,15 KG"],
        ["7. Miktar", "Quantity", "Quantité", "2280 KGS"],
        ["Weight: 1500 KG"],
    ]
    
    for test in weight_tests:
        result = extract_info(test, information_type='weight')
        print(f"\nInput:  {test}")
        print(f"Output: {result}")
    
    # Test item extraction
    print("\n" + "="*70)
    print("ITEM EXTRACTION (NEW)")
    print("="*70)
    ...
    text_list_1 = [
        ...
        "Numéro d'ordre – Marques, numéros, nombre et nature des colis – Désignation des marchandises",
        "",
        "ADDR:10 PACKAGE 21-01761",
        "1",
        "Endüstriyel Mutfak Malzemesi TURKEY",
        "SANTOS NO:33 BAR BLENDIR FRANCE",
        "ROBOT COUPE MP 450 ULTRA EL BLENDIRI FRANCE",
        "LAINOX ICET051 KONVEKSIYONLU FIRIN ELK. ITALY",
        "SCOTSMAN AC 176 AS BUZ MAKINASI ITALY",
        "SANTOS NO:50C KATI MEYVE SIKMA MAKINASI FRANCE",
        "SANTOS NO:11 PORTAKAL SIKMA MAKINASI FRANCE",
        "SANTOS NO:62 BLENDIR FRANCE",
    ]
    items_1 = extract_items(text_list_1)
    print("Items found:")
    for item in items_1:
        print(f"  - {item}")
    ...
    text_list_2 = [
        ...
        "2 PALLETS",
        "HEAT RECOVERY UNIT",
        "RECTANGLE AIR DUCT TYPE FAN",
    ]
    items_2 = extract_items(text_list_2)
    print("Items found:")
    for item in items_2:
        print(f"  - {item}")
    ...
    text_list_3 = [
        ...
        "",
        "ADDR// 27 KAP",
        "FATURA MUHTEVİYATI MALZEMELER",
        "INVOICE NO : OKY2020000000009 - OKY2020000000010",
    ]
    items_3 = extract_items(text_list_3)
    print("Items found:")
    for item in items_3:
        print(f"  - {item}")
    
    # Test with OCR errors in headlines
    print("\nExample 4 (With OCR errors):")
    text_list_4 = [
        "6. Sira No kolilerin marka ve isaretleri sayi ve turleri esyanin tanimi",  # OCR errors
        "Item numbcr marks numbers numbcr and kind ol packages description ol goods",  # OCR errors
        "",
        "MACHINE PARTS FROM GERMANY",
        "INDUSTRIAL EQUIPMENT",
    ]
    items_4 = extract_items(text_list_4, threshold=0.35)  # Higher threshold for OCR errors
    print("Items found (with OCR errors in headlines):")
    for item in items_4:
        print(f"  - {item}")
    
    # Test with countries parameter to filter languages
    print("\nExample 5 (Using countries parameter - REALISTIC WORKFLOW):")
    print("Step 1: Extract countries from the 'Country of Origin' section")
    country_section = ["3. Menşe Ülkesi", "Country of Origin", "FRANCE, ITALY"]
    countries_detected = extract_info(country_section, information_type='country')
    print(f"  Detected countries: {countries_detected}")
    
    print("\nStep 2: Extract items from the 'Items' section using detected countries")
    items_section = [
        "6. Sıra No; kolilerin marka ve işaretleri, sayı ve türleri; eşyanın tanımı",
        "Item number; marks, numbers, number and kind of packages; description of goods",
        "Numéro d'ordre – Marques, numéros, nombre et nature des colis – Désignation des marchandises",
        "",
        "MACHINE PARTS FRANCE",
        "EQUIPMENT ITALY",
        "INDUSTRIAL COMPONENTS",
    ]
    items_with_countries = extract_items(items_section, countries=countries_detected)
    print(f"  Items found: {len(items_with_countries)}")
    for item in items_with_countries:
        print(f"    - {item}")
    
    # Determine which languages were checked
    languages_checked = set(['english'])  # Always checked
    for country in countries_detected:
        if country in COUNTRY_TO_LANGUAGE:
            languages_checked.add(COUNTRY_TO_LANGUAGE[country])
    print(f"  Languages checked: {', '.join(sorted(languages_checked))}")
    
    # Compare: without countries parameter
    print("\nExample 6 (Without countries parameter):")
    items_without_countries = extract_items(items_section, countries=None)
    print(f"  Items found: {len(items_without_countries)}")
    for item in items_without_countries:
        print(f"    - {item}")
    print(f"  Languages checked: all ({len(ITEM_HEADLINES)} languages)")
    
    # Show the efficiency difference
    print("\nExample 7 (Efficiency Comparison):")
    print("  WITH countries=['FRANCE', 'ITALY']:")
    print("    → Checks: English, French, Italian (3 languages)")
    print("    → More efficient for targeted matching")
    print("\n  WITHOUT countries:")
    print(f"    → Checks: All {len(ITEM_HEADLINES)} languages")
    print("    → Still works, but less efficient")
